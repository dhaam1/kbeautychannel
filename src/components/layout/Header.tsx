...
export default function Header({ isDarkBackground = false }: HeaderProps) {
  ...
  const borderColor = isDarkBackground ? 'border-white' : 'border-gray-900';
  const iconColor = isDarkBackground ? 'text-white/50' : 'text-gray-400';
      ...
      <div className="flex items-center gap-8">
        <nav className={`flex items-center gap-6 font-sans text-sm font-semibold ${textColor} tracking-widest uppercase relative z-50`}>
          <div className="relative group">
            <span className="hover:opacity-70 transition-opacity flex items-center gap-1.5 py-4 cursor-pointer">
              전문 시술 분야
              <svg className={`w-3.5 h-3.5 ${iconColor} group-hover:${textColor} transition-transform duration-300 group-hover:rotate-180`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 w-36">
              <div className="bg-white/90 backdrop-blur-xl border border-gray-100 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-2 flex flex-col gap-0.5">
                {[
                  { name: '리프팅', path: '/procedures/thermage' },
        ...
        <button className={`px-5 py-2 rounded-full border ${borderColor} text-sm font-semibold ${textColor} hover:bg-white hover:text-black transition-colors`}>
          시술 예약
        </button>
      </div>
    </header>
