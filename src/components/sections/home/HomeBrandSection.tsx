...
const BRAND_ITEMS = [
  {
    ...
    image: ASSETS.BRAND.ITEM_01,
    alt: '자연스러운 미소를 위한 블랑쉬치과 Natural 컨셉 이미지',
    darkText: true,
  },
  {
    key: 'masterpieces' as const,
    image: ASSETS.BRAND.ITEM_02,
    alt: '작품처럼 완성된 미소를 상징하는 Masterpieces 이미지',
    darkText: false,
  },
  {
    key: 'trust' as const,
    image: ASSETS.BRAND.ITEM_03,
    alt: '평생 함께할 치아를 위한 신뢰를 상징하는 Trust 이미지',
    darkText: false,
  },
] as const;
...
export function HomeBrandSection() {
  ...
  const footerTextLines = translations.raw('footer.text.desktop') as string[] || [];

  return (
    <section className="w-full flex flex-col bg-white">
      <div className="h-auto w-full flex flex-col bg-white">

        {/* 상단 3단 컬럼 영역 - 데스크톱: 가로 배치(세로 크롭), 모바일: 세로 배치(가로 풀) */}
        <div className="w-full mx-auto flex flex-col md:flex-row md:gap-5 max-w-content md:mt-5">
          {BRAND_ITEMS.map((item, index) => {
            const title = translations(
              `${item.key}.title` as 'natural.title' | 'masterpieces.title' | 'trust.title'
            );
            const description = translations.raw(
              `${item.key}.description.default` as
              | 'natural.description.default'
              | 'masterpieces.description.default'
              | 'trust.description.default'
            ) as string[];
            const textColor = item.darkText ? 'text-black' : 'text-white';

            return (
              <div
                key={item.key}
                className="relative flex-1 overflow-hidden min-h-[578px] md:aspect-[3/4]"
              >
                {/* 배경 이미지 */}
                <Image
                  src={item.image}
                ...
                {/* 텍스트 오버레이 */}
                <div className="relative z-10 flex flex-col items-center pt-10 md:pt-12 px-4">
                  <h3
                    className={`text-[28px] md:text-[32px] font-normal tracking-[-0.32px] uppercase ${textColor}`}
                  >
                    {title}
                  </h3>
                  <p
                    className={`mt-3 text-center text-[14px] md:text-[16px] font-normal leading-relaxed tracking-[-0.16px] ${textColor} opacity-80`}
                  >
                    {description.map((line, i, arr) => (
        ...
        <div className="h-auto w-full flex flex-col">
          <AuroraBackground className="h-[400px] md:h-[367px] bg-white">
            <div
              className="w-full mx-auto flex justify-center items-start overflow-hidden pt-[85px] md:pt-[85px] pb-[92px] md:pb-[92px] max-w-content"
            >
              <div className="flex flex-col items-center text-center w-full">
                {/* 메인 슬로건 카피 */}
                <motion.p
                  className="pb-5 text-[32px] md:text-[38px] relative z-10"
                  {...fadeUp({ delay: 0.1 })}
                  style={{
                    color: '#000000',
                    fontStyle: 'normal',
                ...
                >
                  Where Smile <br className="md:hidden" />Becomes Masterpiece.
                </motion.p>

                {/* 한글 서브 카피 */}
                <motion.p
                  className="pb-16 relative z-10 text-[18px] md:text-[24px]"
                  {...fadeUp({ delay: 0.15 })}
                  style={{
                    color: '#000000',
                    fontStyle: 'normal',
                ...
                >
                  {footerTextLines.map((line, index, array) => (
