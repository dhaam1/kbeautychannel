...
import { LaminateHeroSparkles } from '@/components/sections/laminate/LaminateHeroSparkles';

/**
 * 라미네이트 히어로 섹션 컴포넌트
 * 
 * @description
 * 라미네이트 페이지의 최상단 히어로 섹션입니다.
 * 배경 이미지만 표시합니다.
 * 
 * 레이아웃 구조:
 ...
 * 
 * @returns {Promise<JSX.Element>} 히어로 섹션 컴포넌트
 */
export async function LaminateHeroSection() {
  const t = await getTranslations('solution.laminate.hero.textLayout');
  const tButton = await getTranslations('solution.laminate.hero.button');
    ...
    <section className="relative w-full h-[300svh] md:h-[340svh]">
      ...
      >
        ...
        <div className="absolute inset-0 z-0">
          {/* 모바일 배경 이미지 */}
          <Image
            src={ASSETS.LAMINATE.HERO_BG_MOBILE}
            alt=""
            fill
        ...
        <Container className="relative h-full">
          <div className="relative h-full flex flex-col md:flex-row items-center md:items-end justify-end md:justify-between pb-[87px] md:pb-[114px] gap-[27px]">
            ...
            <div className="z-20">
              <HeroConsultationButton label={tButton('text')} />
            </div>
          </div>
        </Container>
      </div>
