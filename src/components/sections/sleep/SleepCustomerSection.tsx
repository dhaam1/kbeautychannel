...
/**
 * 수면 치료 안전성 섹션 컴포넌트
 * 디자인 시스템(Border, Font Size, Spacing) 통일 적용
 */
export function SleepCustomerSection() {
  const { fadeUp } = useResponsiveAnimation();
  const labelMotion = fadeUp({ delay: 0.1 });
  // const titleMotion = fadeUp({ delay: 0.15 }); // 사용하지 않는 변수 주석 처리
    ...
    <section className="w-full bg-white">
      <Container>
        <div className="py-[60px] md:py-[80px]">
          ...
          {/* 4개 컬럼 그리드 */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-[34px]">
            {safetyFeatures.map((feature, index) => (
              ...
              >
                <div className="flex flex-col w-full h-full">
                  {/* 이미지 영역 */}
                  <div
                    className="relative w-full aspect-[300/220] bg-gray-100 overflow-hidden"
                  >
                    <Image
                      src={feature.image}
                      alt={feature.title}
                  ...
                  {/* 텍스트 영역 */}
                  <div
                    // [디자인 시스템 적용] 흰색 배경, 패딩 20px
                    className="flex flex-col flex-grow p-[20px] bg-white"
                  >
                    {/* 제목 */}
                    <h3
                      style={{
                        color: '#262626',
                        fontSize: '20px', // [통일] 20px
                        fontWeight: 500,
                        lineHeight: '1.2',
                        letterSpacing: '-0.2px',
                        marginBottom: '12px',
